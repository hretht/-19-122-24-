...
# Добавляем пользователя "Атабаев Тимур"
try:
    cursor.execute("INSERT INTO users (username, first_name, last_name, password) VALUES (?, ?, ?, ?)",
                   ("atabaevtimur", "Атабаев", "Тимур", "pass987"))
    conn.commit()
except sqlite3.IntegrityError:
    pass

# Заполняем таблицу автомобилей информацией (переведено на русский)
default_cars = [
    (
        "Lada Largus",
        "Lada (АвтоВАЗ)",
...
def open_car_selection():
    ...
    tab2 = ttk.Frame(car_window)
    tab3 = ttk.Frame(car_window)
...
def user_login():
    ...
    # Если введены логин "atabaevtimur" и пароль "pass987", выводим нужное сообщение
    if username == "atabaevtimur" and password == "pass987":
        messagebox.showerror("Ошибка", "Атабаевым ВХОД ЗАПРЕЩЕН ПОШЕЛ НАХУЙ")
        return
